...
const OctomizePanel = ({ totalRuns, targetRowData, benchmarkChecked, runsPerTrial, numTrials, isOctomizeDisabled, watch, isAccelerateComplete }: Props) => {
  ...
  return (
    <Paper sx={{ borderRadius: '8px', padding: '24px', position: 'relative' }} elevation={6}>
        ...
        {targetRowData.map((targetRow, index) => (
          targetRow.instance && (<Box key={index} sx={{ display: 'flex' }}>
            <Grid container spacing={0} sx={{ marginBottom: '14px' }}>
              <Grid item xs={9}>
                <Typography sx={{ fontWeight: 600, fontSize: '16px', marginBottom: '0px', display: 'inline' }} variant='body1'>
                  {targetRow.instance}
                {isAccelerateComplete && (watch('accelerateHardware') === targetRow.provider) && <FlashAutoIcon sx={{color: 'orange', margin: '0 0 0 4px', width: '16px', height: '16px'}} />}
                </Typography>
      ...
      <Button name={'octomize'} sx={{ width: '100%', height: '48px', borderRadius: '8px', textTransform: 'none' }} type="submit" disabled={!isOctomizeDisabled} variant="contained">Octomize</Button>
    </Paper>
  )
